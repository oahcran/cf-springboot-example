...
sleep 60  # wait for java process to start

JAVA_HOME=$HOME/.java-buildpack/open_jdk_jre
LOOP_WAIT=10  # how long to wait in seconds

# pick the pid of our Java process
echo "Searching for the Java process..."
ps axf
PID=$(ps axf | grep "org.springframework.boot.loader.JarLauncher" | grep -v grep | awk '{print $1}')
